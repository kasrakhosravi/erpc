...
- Auxiliary JS workspace: pnpm 10 (typed config under `@erpc-cloud/config`)
...
make test-fast      # faster, non-race
...
- See [.cursor/rules/erpc.md](.cursor/rules/erpc.md) for the canonical project rules — applies to all agents, not just Cursor.
- Code style: idiomatic Go, no surprises. `gofmt`/`make fmt` is the source of truth.
